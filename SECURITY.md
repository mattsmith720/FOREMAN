...
## What is locked down on this branch

| Control | Implementation | Evidence on this branch |
|---|---|---|
| **Secrets in git** | `.env` gitignored; gitleaks runs on every push and PR with full git history. | `.github/workflows/gitleaks.yml`, `.gitleaks.toml`. Local `gitleaks detect` on HEAD + 34-commit history: zero leaks. |
...
| **Rate limiting** | Per-IP via `X-Forwarded-For`; 120/min global; tighter on AI routes (`/analyse` 20/min, `/transcribe` 30/min, `/voice/speak` 30/min, `/voice/advice` 15/min). | `backend/src/index.ts` + `backend/src/routes/*`; integration test in `backend/src/rate-limit.test.ts` proves the (cap+1)th call returns `429` for each route. |
...
| Variable | Required |
|----------|----------|
| `BACKEND_URL` | Yes — Render API URL. Server-side only. |
...
- The `*.vercel.app` allowance is intentional so preview deployments work.
  ...
  `web/middleware.ts`.
- AU privacy / customer consent flow not complete — see `CLAUDE.md` before any
  live pilot with customers.
